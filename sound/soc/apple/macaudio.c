...
static const struct macaudio_vlimit macaudio_safe_volumes[] = {
	[AMP_TAS5770] = { TAS5770_0DB, 1 },
	[AMP_SN012776] = { SN012776_0DB, 1 },
	[AMP_SSM3515] = { SSM3515_0DB, 1 },
};
...
static void macaudio_vlimit_unlock(struct macaudio_snd_data *ma, bool unlock)
{
	...
	if (!name || ma->cfg->amp == AMP_NONE) {
		WARN_ON_ONCE(1);
		return;
	}

	if (unlock)
		max = macaudio_safe_volumes[ma->cfg->amp].unlocked_max;
	else
		max = macaudio_safe_volumes[ma->cfg->amp].locked_max;
					 ...
					 int be_index, int ncodecs_per_cpu)
{
	...
	/*
	 * The codecs we have only support zero-fill (tas2764) or pulldown (tas2770).
	 * They reset to HIZ, so NONE = HIZ, which is already set.
	 */
	...
	ret = of_property_read_u32(np, propname, &idle->idle_mask);
	if (ret)
		return ret;
				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	/* Keep existing BE rate for sense PCM (0 if not already running) */
	if (props->is_sense ||
		(props->is_speakers && substream->stream == SNDRV_PCM_STREAM_CAPTURE)) {
		rate->min = rate->max = cpu_dai->symmetric_rate;
		return 0;
	}

	/* Speakers playback BE */
...
static int macaudio_slk_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *uvalue)
{
	...
	ma->speaker_lock_remain = ms_to_ktime(SPEAKER_LOCK_TIMEOUT_MS);
	ma->speaker_lock_timeout = ktime_add(ktime_get(), ma->speaker_lock_remain);
	macaudio_vlimit_update(ma);
